...
def setRestricoes(model, dados):
        ...
        for d in range(dados.D_len - 1):
            for t in dados.T:
                if (isinstance(dados.get_R_t(t), list) == False):
                    model.m.addConstr(model.x_idt[i, d, dados.index_T(t)] + model.x_idt[i, d+1, dados.index_T(dados.get_R_t(t))]
                                      <= 1, name="h_cons_02["+str(i)+"]["+str(d)+"]["+str(t)+"]["+str(dados.get_R_t(t))+"]")
                else:
    ...
    for i in dados.I:
        for t in dados.T:
            model.m.addConstr((gp.quicksum(model.x_idt[dados.index_I(i), d, dados.index_T(t)] for d in range(
                dados.D_len)) <= dados.get_m_it_max(i, t)), name="h_cons_03["+str(i)+"]["+str(t)+"]")
    ...
    for i in dados.I:
        for d in range(dados.D_len - dados.get_c_i_max(i)):
            model.m.addConstr((gp.quicksum(model.x_idt[dados.index_I(i), j, t] for j in range(d, d+dados.get_c_i_max(i)+1)
                                           for t in range(dados.T_len)) <= dados.get_c_i_max(i)), name="h_cons_06["+str(i)+"]["+str(d)+"]")

    # Hard Constraint 07
    for i in dados.I:
        for c in range(1, dados.get_c_i_min(i)):
            for d in range(dados.D_len - (c+1)):
                model.m.addConstr((gp.quicksum(model.x_idt[dados.index_I(i), d, t] for t in range(dados.T_len))+c-1-(gp.quicksum(model.x_idt[dados.index_I(i), j, t] for j in range(d+1, d+c+1) for t in range(
                    dados.T_len))) + gp.quicksum(model.x_idt[dados.index_I(i), d+c+1, t] for t in range(dados.T_len)) >= 0), name="h_cons_07["+str(i)+"]["+str(c)+"]["+str(d)+"]")

    # Hard Constraint 08
    for i in dados.I:
        for b in range(1, dados.get_o_i_min(i)):
            for d in range(dados.D_len - (b+1)):
                model.m.addConstr((1 - gp.quicksum(model.x_idt[dados.index_I(i), d, t] for t in range(dados.T_len)) + gp.quicksum(model.x_idt[dados.index_I(i), j, t] for j in range(d+1, d+b+1) for t in range(
                    dados.T_len)) - gp.quicksum(model.x_idt[dados.index_I(i), d+b+1, t] for t in range(dados.T_len)) >= 0), name="h_cons_08["+str(i)+"]["+str(b)+"]["+str(d)+"]")
    ...
    for i in dados.I:
        ...
        for w in range(dados.W_len):
            model.m.addConstr((model.k_iw[dados.index_I(i), w] <= gp.quicksum(model.x_idt[dados.index_I(i), (7*w)+5, t] for t in range(dados.T_len)) +
                               gp.quicksum(model.x_idt[dados.index_I(i), (7*w)+6, t] for t in range(dados.T_len))), name="h_cons_09.1["+str(i)+"]["+str(w)+"]")

            model.m.addConstr((gp.quicksum(model.x_idt[dados.index_I(i), (7*w)+5, t] for t in range(dados.T_len)) + gp.quicksum(model.x_idt[dados.index_I(
                i), (7*w)+6, t] for t in range(dados.T_len)) <= 2*model.k_iw[dados.index_I(i), w]), name="h_cons_09.2["+str(i)+"]["+str(w)+"]")
    ...
    for i in dados.I:
        for d in dados.D:
            for t in dados.T:
                model.m.addConstr((dados.get_q_idt(i, d, t)*(1-model.x_idt[dados.index_I(i), dados.index_D(d), dados.index_T(t)])+dados.get_p_idt(i, d, t)*(model.x_idt[dados.index_I(
                    i), dados.index_D(d), dados.index_T(t)])) == model.v_idt[dados.index_I(i), dados.index_D(d), dados.index_T(t)], name="s_cons_01["+str(i)+"]["+str(d)+"]["+str(t)+"]")

    # Soft Constraint 02
    for d in dados.D:
        for t in dados.T:
            model.m.addConstr((gp.quicksum(model.x_idt[i, dados.index_D(d), dados.index_T(t)] for i in range(dados.I_len))-model.z_dt[dados.index_D(
                d), dados.index_T(t)] + model.y_dt[dados.index_D(d), dados.index_T(t)]) == dados.get_u_dt(d, t), name="s_cons_02["+str(d)+"]["+str(t)+"]")

    model.m.update()
    return


def printSolucao(model, instancia, tempo):
    output = ""
...
def solucaoTabela(model, dados, instancia):
    output = ""
...
def resolverGurobi(model, dados, instancia):
...
def criaDiretorios():
    ...
    for i in range(1, 25):
        if not os.path.exists(subdiretorio + '/' + str(i)):
            os.makedirs(subdiretorio + '/' + str(i))
